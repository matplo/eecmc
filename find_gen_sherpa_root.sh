...
do
	...
	output_file="${output_file}_${flav}_${ecm}_${jet_setting}_ana_eec.txt"
	# echo ${input_file} | tee -a ${output_file}
	root_input_file=$(dirname ${input_file})/$(basename ${input_file} .hepmc)_ana_eec.root
	if [ ! -e ${root_input_file} ]; then
		echo "File ${root_input_file} not found"
		continue
	fi
	echo ${root_input_file} | tee -a ${output_file}
done
