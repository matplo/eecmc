...
class JetAnalysis(AnalysisBase):
    ...
    def init(self):
        ...
        self.tn_jet 	= ROOT.TNtuple(f'tn_jet_{self.name}', 		'tn_jet', 		'nev:nj:ij:pt:eta:phi:m:xsec:ev_weight')
        self.tn_parts 	= ROOT.TNtuple(f'tn_parts_{self.name}', 	'tn_parts', 	'nev:idx:nj:ij:pt:eta:phi:m:pid:status:xsec:ev_weight')
    ...
    def select_pythia_particles(self, pythia):
        ...
        if self.cfg.parts_select_parton:
            self.gen_parts = [(i, p, p.id(), p.status()) for i, p in enumerate(pythia.event) if (p.isParton() and (p.isFinal() or (71 <= abs(p.status()) <= 79)))]
            ...
            if self.cfg.parts_select_final:
                if self.cfg.parts_select_charged:
                    self.gen_parts = [(i, p, p.id(), p.status()) for i, p in enumerate(pythia.event) if p.isFinal() and p.isCharged()]
                else:
                    self.gen_parts = [(i, p, p.id(), p.status()) for i, p in enumerate(pythia.event) if p.isFinal()]
            else:
                self.gen_parts = [(i, p, p.id(), p.status()) for i, p in enumerate(pythia.event)]
                if self._nev < 1:
                    self._log.warning(f'{self.name}: selecting all particles final and not final state!')
        for xp in self.gen_parts:
    ...
    def run_jet_analysis(self):
        ...
        for ij, j in enumerate(self.jets):
            ...
            self.tn_jet.Fill(self._nev, self.njet_count + ij, ij, j.pt(), j.eta(), j.phi(), j.m(), self.xsec, self.ev_weight)
        if self.cfg.write_parts:
            for p, py_p in zip(self.parts, self.gen_parts):
                njet = -1
                ...
                if len(ijet) < 1:
                    ...
                    ijet = ijet[0]
                    njet = self.njet_count + ijet[0]
                self.tn_parts.Fill(self._nev, p.user_index(), njet, ijet, p.pt(), p.eta(), p.phi(), p.m(), py_p[2], py_p[3], self.xsec, self.ev_weight)
...
class JetChargedFullAnalysis(AnalysisBase):
    ...
    def analyze_hepmc_event(self, hepmc):
        rv = True
        if self.cfg.parton_hadron:
            rv = rv and self.j_parton_ana.analyze_event(hepmc=hepmc, nev=self._nev)
        if not rv:
            return False
        rv = rv and self.j_ch_ana.analyze_event(hepmc=hepmc, nev=self._nev)
        if not rv:
            return False
        rv = rv and self.j_full_ana.analyze_event(hepmc=hepmc, nev=self._nev)
        if not rv:
            return False
