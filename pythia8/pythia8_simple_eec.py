...
def main():
  ...
  tn_eec_pcone = {}
  for ptcut in pt_cuts:
    tn_eec_pcone[ptcut] = ROOT.TNtuple(f'tn_eec_pcone_ptcut{ptcut}', 'tn_eec_pcone_ptcut{ptcut}', 'nev:xsec:ev_weight:ij:dr:pt1:pt2:eec:ptjet:ptcut:iidx:jidx:ptlead:pid')
        ...
        for ptcut in pt_cuts:
            ...
            for pcone in [_parts_cut_pcone_plus, _parts_cut_pcone_minus]:
              ...
              for first, second in _pairs_pcone:
                  ...
                  eec = first.perp() * second.perp() / pow(j.perp(), 2.)
                  # tn_eec[ptcut] = ROOT.TNtuple(f'tn_eec_ptcut{ptcut}', 'tn_eec_ptcut{ptcut}', 'nev:xsec:ev_weight:ij:dr:pt1:pt2:eec:ptjet:ptcut:iidx:jidx:ptlead')
                  tn_eec_pcone[ptcut].Fill(iev, sigmaGen, ev_weight, ij, dr, first.perp(), second.perp(), eec, j.perp(), ptcut, first.user_index(), second.user_index(), ptlead, pid)
    else:
      continue
