...
if [[ "$file_input" == *"_10"* ]]; then
	mode=10
fi

if [[ "$file_input" == *"_15"* ]]; then
	mode=15
fi

echo "mode: $mode because file_input: $file_input"

if [[ "$file_input" == *"_10"* ]]; then
	file_output10="${file_output%.*}_10.root"
	lre "$template_config" --define input="$file_input" output="$file_output10" jet_min_pt=10 jet_max_pt=15 > "$tmp_file"
	draw_from_yaml.py -c $tmp_file
fi

if [[ "$file_input" == *"_15"* ]]; then
	file_output15=${file_output%.*}_15.root
	lre $template_config --define input=$file_input output=$file_output15 jet_min_pt=15 jet_max_pt=30 > $tmp_file
	draw_from_yaml.py -c $tmp_file
fi
